...
const ui = {
    ...
    initRoleRevealControls() {
        const card = document.getElementById("roleCard");
        if (!card || card.dataset.peekBound === '1') return;
        card.dataset.peekBound = '1';

        const getMode = () => state.config.roleRevealMode || 'standard';
        const isRoleScreen = () => document.getElementById('roleSection')?.classList.contains('active');

        const resetLift = () => {
            card.classList.remove('is-dragging', 'peek-open');
            card.style.setProperty('--front-lift', '0px');
        };

        const finishPeek = () => {
            const peek = this.rolePeekState;
            if (!peek.active) return;
            const didReveal = peek.didReveal;
            peek.active = false;
            peek.pointerId = null;
            card.classList.remove('is-dragging');
            card.classList.remove('peek-open');
            card.style.setProperty('--front-lift', '0px');
            ...
            if (didReveal) {
                window.setTimeout(() => {
                    if (state.currentIndex < state.players.length) {
                        ...
                        if (window.anim) anim.pulseGlow(card);
                    }
                }, 360);
            }

            peek.didReveal = false;
            peek.maxLift = 0;
            peek.suppressClickUntil = Date.now() + 380;
        };

        card.addEventListener('pointerdown', (e) => {
            if (getMode() !== 'lift' || !isRoleScreen()) return;
            e.preventDefault();
            const peek = this.rolePeekState;
            peek.active = true;
            ...
            peek.didReveal = false;
            peek.maxLift = 0;
            card.classList.add('slide-mode', 'is-dragging');
        ...
        window.addEventListener('pointermove', (e) => {
            ...
            const deltaY = e.clientY - peek.startY;
            const maxLift = Math.max(140, Math.min(card.clientHeight - 42, 300));
            const lift = Math.max(0, Math.min(maxLift, -deltaY));
            peek.maxLift = Math.max(peek.maxLift, lift);
            card.style.setProperty('--front-lift', `${-lift}px`);

            if (lift > 8) {
                card.classList.add('peek-open');
            } else {
                card.classList.remove('peek-open');
            }

            if (!peek.didReveal && lift >= 22) {
                peek.didReveal = true;
                this.playRoleRevealFx(state.roles[state.currentIndex]);
            }
        }, { passive: false });
        ...
        window.addEventListener('pointercancel', () => finishPeek());
        window.addEventListener('blur', () => {
            if (this.rolePeekState.active) finishPeek();
        });

        this.resetRolePeekCard = resetLift;
    },

    refreshRoleRevealMode() {
        if (document.getElementById('roleSection')?.classList.contains('active') && state.players.length > 0) {
            this.updateTurnDisplay();
        }
    },

    playRoleRevealFx(role) {
        if (role === "Spy") {
            state.audio.playSound('glitch');
    ...
    updateInputMode(triggeredByToggle = false) {
        ...
        if (triggeredByToggle) state.saveGame();
    },
    ...
    showSystemSettings() {
        ...
        setVal("roleRevealMode", state.config.roleRevealMode || 'standard');
        setVal("themeSelect", state.config.theme);
        
        document.getElementById("spyHintToggle").checked = state.config.spyHintEnabled;
        document.getElementById("rememberLocalDataToggle").checked = !!state.config.rememberLocalData;
    ...
    updateTurnDisplay() {
        ...
        document.getElementById("nextPlayerBtn").style.display = "none";
        const spyRevealActions = document.getElementById("spyRevealActions");
        if (spyRevealActions) spyRevealActions.innerHTML = "";

        if (this.roleTurnRenderTimeout) {
            clearTimeout(this.roleTurnRenderTimeout);
            this.roleTurnRenderTimeout = null;
        }

        const finalizeTurnCard = () => {
            this.buildRoleRevealContent();
            if (this.resetRolePeekCard) this.resetRolePeekCard();
            if (window.anim) anim.pop(card);
        };

        card.classList.remove("peek-open", "is-dragging");
        card.style.setProperty('--front-lift', '0px');

        if (revealMode === 'lift') {
            card.classList.add("slide-mode");
            card.classList.remove("flipped");
            front.innerHTML = `<div class="role-icon"><i class="fas fa-angles-up"></i></div>`;
            finalizeTurnCard();
            return;
        }

        card.classList.remove("slide-mode");
        front.innerHTML = `<div class="role-icon"><i class="fas fa-fingerprint"></i></div>`;

        if (wasFlipped) {
            if (back) back.innerHTML = "";
            card.classList.remove("flipped");
            this.roleTurnRenderTimeout = setTimeout(() => {
                this.roleTurnRenderTimeout = null;
                finalizeTurnCard();
            }, 420);
        } else {
