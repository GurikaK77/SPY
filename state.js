...
const state = {
    ...
    saveGame() {
        if (!this.config.rememberLocalData) {
            localStorage.removeItem('spyGameState');
            return;
        }

        const manualPlayers = Array.isArray(this.savedManualPlayers) ? this.savedManualPlayers : [];
        const gameState = {
            config: this.config,
            savedManualPlayers: manualPlayers,
            players: this.config.manualEntry ? manualPlayers : [],
    ...
    loadGame() {
        ...
        try {
            ...
            this.savedManualPlayers = Array.isArray(s.savedManualPlayers) ? s.savedManualPlayers : [];
            this.players = this.config.manualEntry ? [...this.savedManualPlayers] : [];
            this.roles = [];
            this.chosenWordObj = { w: "", h: "" };
            this.chameleonWordObj = { w: "", h: "" };
            this.currentIndex = 0;
            this.timeLeft = 0;
            this.isDetectiveMode = false;
            this.isPointsEnabled = this.config.pointsSystem === 'enabled';

            this.gameStats = s.gameStats || this.gameStats;
            this.dailyChallenges = s.dailyChallenges || this.dailyChallenges || [];
    ...
    updateConfig(key, value) {
        ...
        if (['spyHintEnabled', 'modifierInfection', 'modifierBlackout', 'rememberLocalData'].includes(key)) value = !!value;
