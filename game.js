...
const game = {
    ...
    endGame() {
        ...
        if(state.isPointsEnabled || state.config.modifierInfection) {
            ...
            this.revealSpies(false, false, { deferredSpyReveal: !!state.config.manualEntry });
        }
    },
    ...
    revealSpies(calculated = false, forceNoText = false, options = {}) {
        state.audio.playSound('reveal');
        
        const deferredSpyReveal = !!options.deferredSpyReveal;
        ...
        if (state.config.manualEntry) {
            if (deferredSpyReveal) {
                revealHtml = `<div class="spy-reveal-container spy-reveal-placeholder"><div class="spy-label">დრო ამოიწურა</div></div>`;
            } else {
                revealHtml = `
        ...
        if (spyRevealActions) {
            spyRevealActions.innerHTML = "";
            if (deferredSpyReveal && state.config.manualEntry) {
                spyRevealActions.innerHTML = `
                    <button class="btn spy-reveal-btn" id="revealSpyBtn" onclick="game.revealSpyNameNow()">
                        <i class="fas fa-user-secret"></i> ჯაშუშის ნახვა
                    </button>
                `;
            }
        }
    ...
    revealSpyNameNow() {
        ...
        const spyRevealActions = document.getElementById("spyRevealActions");
        if (resultDisplay) {
            const existingExtras = resultDisplay.innerHTML;
            resultDisplay.innerHTML = existingExtras.replace(
                /<div class="spy-reveal-container spy-reveal-placeholder">[\s\S]*?<\/div>/,
                `<div class="spy-reveal-container"><div class="spy-label">ჯაშუში არის</div><div class="spy-name-big">${spies || "ვერ მოიძებნა"}</div></div>`
            );
        }
